...
<script lang="ts">
import { Component, Prop, Watch, Getter, mixins } from 'nuxt-property-decorator';
...
export default class ComponentCustomerLegalForm extends mixins(validationMixin) {
    @Getter('countries-list/getCountriesList') countries!: ICountryList;
    @Getter('customer/getError') error!: string;

    @Prop() public formData!: ICustomer;
    ...
    public handleFormData(val: ICustomer): void {
        if (this.formData !== null) {
            const { firstName, lastName, email, phoneNumber, companyName, companyCode, vatCode } = val;
